...
def generate_dollar_volume_weights(close, volume):
    ...
    # multiplying each volume to its price
    product_close_volume = np.multiply( volume.values,close.values)
    
    # the ndaray is converted to a df
    product_close_volume_df = pd.DataFrame(product_close_volume, columns = volume.columns, index = volume.index)
    
    # summing the values in the df
    product_close_volume_df_sum = product_close_volume_df.sum(axis=1)
    
    # calculating the dollar volume weights
    dollar_volume_weights = product_close_volume_df.div(product_close_volume_df_sum, axis=0) 
...
def calculate_dividend_weights(dividends):
    ...
    # getting the cummulative sum of all dividends
    ex_dividends = np.cumsum(dividends)
    
    # the ex_dividends df
    ex_dividends_df = pd.DataFrame(ex_dividends, columns = dividends.columns, index = dividends.index)
    
    # summing the values in the df
    ex_dividends_df_sum = ex_dividends_df.sum(axis=1)
    
    # calculating the dividend weight for each dividend
    dividend_weights = ex_dividends_df.div(ex_dividends_df_sum, axis =0)
...
def generate_returns(prices):
    ...
    # getting  the Percentage change between the current and a previous price
...
def generate_weighted_returns(returns, weights):
    ...
    # multiplying each return to its weight
    product_return_weight = np.multiply( returns.values, weights.values)
    
    # the weighted return is converted to a df
    weighted_returns = pd.DataFrame(product_return_weight, columns = returns.columns, index = returns.index)
...
def calculate_cumulative_returns(returns):
    ...
    # sum returns for the returns
    ret_returns = 1+returns.sum(axis=1)
    
    # comulative product returns for the returns
    cumulative_returns = ret_returns.cumprod()
...
def tracking_error(benchmark_returns_by_date, etf_returns_by_date):
    ...
    error_diff = etf_returns_by_date.subtract(benchmark_returns_by_date).std()
    
    # annual tracking error
    tracking_error = np.sqrt(252)*error_diff
...
def get_covariance_returns(returns):
    ...
    # getting the number of stocks and replacing nan values with 0
    num_stocks = returns.fillna(0)
    
    # calculating covariance
    returns_covariance = np.cov(num_stocks, rowvar=False)
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
   ...
   # reducing the shift size by 1
    s_size = shift_size-1
    
    # gets the length  of the returns
    r_len = len(returns)
    
    # empty list object for all rebalance weigths
    all_rebalance_weights = []
    
    for size in range (s_size, r_len):
        if(size%shift_size == 0) and (size-chunk_size >= 0 ):
            size_ret = returns.iloc[size-chunk_size:size]
            cov_size_ret = get_covariance_returns(size_ret)
            optimal_w = get_optimal_weights(cov_size_ret, index_weights.iloc[size-1])
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    #  empty list for turn over
    p_turn_over = []
    
    # getting the rebalance weight and the length
    bal_weights = all_rebalance_weights
    bal_weights_len = len(bal_weights)
    
    # looping through  and getting the absolute values between the present  and next balance weights 
    for j in range(bal_weights_len-1):
        turns = np.abs((bal_weights[j] - bal_weights[j+1]))
        p_turn_over.append(turns)
     
    # getting the sum of turn overs
    sum_tot_turn_over = np.sum(p_turn_over)
    
    # getting the sum total portfolio turn over
    sum_reb = sum_tot_turn_over/rebalance_count
    trad_days = (n_trading_days_in_year/shift_size)
    
    portfolio_turnover = np.multiply(sum_reb, trad_days)
